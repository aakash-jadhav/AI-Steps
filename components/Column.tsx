  ...
  if (data.type === 'input') {
    return (
      <ColumnContainer className={isHero ? 'md:border-none md:bg-transparent !border-r-0' : '!border-r-0'}>
        <div className={`flex-1 flex flex-col justify-center items-center ${isHero ? 'gap-4' : 'gap-8'} p-4 transition-all duration-500 ${isHero ? '' : 'pt-20'}`}>
          <div className={`
          ...
          `}>
            <Hammer className={`text-white transition-all duration-500 ${isHero ? 'w-10 h-10' : 'w-10 h-10'}`} />
          </div>

          <div className="text-center space-y-2 shrink-0">
            <h1 className={`font-extrabold text-slate-700 tracking-tight transition-all duration-500 ${isHero ? 'text-3xl md:text-4xl' : 'text-2xl'}`}>
            ...
            <p className={`text-slate-500 font-medium transition-all duration-500 ${isHero ? 'text-base' : 'text-sm'}`}>Ask me how to do anything!</p>
          </div>

          <form onSubmit={handleSubmit} className={`w-full space-y-4 pb-12 transition-all duration-500 ${isHero ? 'max-w-2xl' : 'max-w-sm'}`}>
            <ClayTextArea
              ref={textAreaRef}
              ...
              autoFocus
              rows={isHero ? 4 : 6}
              className={`transition-all duration-500 ${isHero ? 'min-h-[80px] text-lg px-4 py-3' : 'min-h-[120px] text-base'}`}
            />
            <ClayButton type="submit" variant="success" className={`w-full text-base ${isHero ? 'py-3' : 'py-4'} shadow-lg`}>
